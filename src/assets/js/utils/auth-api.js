...
// Configuration de l'API (selon le guide d'intégration)
const API_BASE_URL = 'https://earthkingdoms-mc.fr/api';
...
class AuthAPI {
    ...
    async checkToken(token) {
        ...
        try {
            ...
            console.error('[AuthAPI] Erreur lors de la vérification du token:', error);
            return {
                error: true,
                valid: false,
    ...
    getTokenTimeRemaining(expires) {
        ...
        if (expires > 10000000000) {
            ...
            expiresSeconds = Math.floor(expires / 1000);
            console.log('[AuthAPI] Token expires en millisecondes, conversion en secondes:', expiresSeconds);
        } else {
        ...
        // Logs réduits - seulement si proche de l'expiration
        if (remaining < 3600 && remaining > 0) {
            const hours = Math.floor(remaining / 3600);
            const minutes = Math.floor((remaining % 3600) / 60);
            console.log(`[AuthAPI] Temps restant: ${hours}h ${minutes}min`);
        }
        return remaining;
    }
    ...
    isTokenNearExpiration(expires) {
        ...
        return timeRemaining > 0 && timeRemaining < 3600; // Moins de 1 heure (3600 secondes)
    }
    ...
    isTokenInGracePeriod(expires) {
        ...
        const inGrace = timeRemaining < 0 && timeRemaining > -1800;
        return inGrace;
    }
    ...
    isTokenExpiredTooLong(expires) {
        if (!expires) return true;
        const timeRemaining = this.getTokenTimeRemaining(expires);
        const expiredTooLong = timeRemaining <= -1800; // Expiré depuis plus de 30 minutes (1800 secondes)
     ...
     */
    getSkinUrl(pseudo) {
        return `https://earthkingdoms-mc.fr/skins/${pseudo}.png`;
    }
    ...
    convertToAccountFormat(apiData, userInfo = null) {
        ...
        // Générer un UUID déterministe basé sur le pseudo (comme pour les comptes crack)
        ...
        return {
            name: username, // Pseudo depuis la BDD (source de vérité)
            uuid: uuid,
            access_token: token, // Token launcher (64 caractères hex, valable 12 heures)
            token_expires: expires, // Timestamp d'expiration
            meta: {
                type: 'EarthKingdoms', // Type personnalisé pour identifier nos comptes
                online: false, // Mode offline pour Minecraft
                username: username, // Pseudo depuis la BDD
                is_admin: is_admin || 0,
                // Informations utilisateur optionnelles (si disponibles)
